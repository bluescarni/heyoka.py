...
class sgp4_propagator_test_case(_ut.TestCase):
    ...
    def test_basics(self):
        ...
        with self.assertRaises(ValueError) as cm:
            prop.diff_args
        self.assertTrue(
            "The function 'get_diff_args()' cannot be invoked on an sgp4 propagator without derivatives"
            in str(cm.exception)
        )
        ...
        self.assertTrue(
            "The function 'get_dslice()' cannot be invoked on an sgp4 propagator without derivatives"
            in str(cm.exception)
        )
        ...
        with self.assertRaises(ValueError) as cm:
            prop.get_mindex(i=0)
        self.assertTrue(
            "The function 'get_mindex()' cannot be invoked on an sgp4 propagator without derivatives"
            in str(cm.exception)
        )
    ...
    def test_propagation(self):
        ...
        for fp_type in [float, np.single]:
            ...
            self.assertTrue(
                "A times/dates array with 1 or 2 dimensions is expected as an input for the call operator of an sgp4 propagator, but an array with 0 dimensions was provided instead"
                in str(cm.exception)
            )
            ...
            self.assertTrue(
                "A times/dates array with 1 or 2 dimensions is expected as an input for the call operator of an sgp4 propagator, but an array with 3 dimensions was provided instead"
                in str(cm.exception)
            )
            ...
            self.assertTrue(
                ...
                "times/dates array is 5, but the propagator contains 2 satellite(s) instead"
                in str(cm.exception)
            )
            ...
            with self.assertRaises(ValueError) as cm:
                prop(tmp2)
            self.assertTrue(
                "Invalid times/dates array detected as an input for the call operator of an sgp4 propagator: the array is not C-style contiguous"
                in str(cm.exception)
            )
            ...
            self.assertTrue(
                "Invalid output array detected in the call operator of "
                "an sgp4 propagator in batch mode: the first dimension has a size of 9, but a "
                "size of 10 (i.e., equal to the number of evaluations) is required instead"
                in str(cm.exception)
            )
