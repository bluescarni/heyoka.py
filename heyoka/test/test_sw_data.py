...
class sw_data_test_case(unittest.TestCase):
    ...
    def test_custom_data(self):
        ...
        # reserved prefix.
        d2 = sw_data(data=tbl, timestamp="20260101", identifier="custom")
        self.assertEqual(d2.timestamp, "20260101")
        self.assertEqual(d2.identifier, "custom")
        self.assertEqual(d2.table.dtype, sw_data_row)
        ...
        d3 = sw_data(data=nc, timestamp="ts", identifier="custom")
        ...
        for bad in (
            ...
            with self.assertRaises(TypeError) as cm:
                sw_data(data=bad, timestamp="ts", identifier="custom")
        ...
        packed = np.dtype(
            {
                "names": list(sw_data_row.names),
                "formats": ["<f8"] * len(sw_data_row.names),
                "offsets": [sw_data_row.fields[n][1] for n in sw_data_row.names],
                "itemsize": sw_data_row.itemsize,
            }
        )
        self.assertEqual(packed, sw_data_row)
        ...
        d5 = sw_data(data=misp, timestamp="ts", identifier="custom")
