...
conda create -y -p $deps_dir python=3.11 c-compiler cxx-compiler git pybind11 'numpy>=2' \
    ...
    spdlog sympy cloudpickle 'mppp=2.*' numba 'clang<19' 'clangxx<19'
...
cmake ../ -DCMAKE_INSTALL_PREFIX=$deps_dir -DCMAKE_PREFIX_PATH=$deps_dir -DCMAKE_BUILD_TYPE=Debug -DHEYOKA_WITH_SLEEF=yes -DHEYOKA_WITH_MPPP=yes -DHEYOKA_FORCE_STATIC_LLVM=yes -DHEYOKA_HIDE_LLVM_SYMBOLS=yes
