...
conda create -y -q -p $deps_dir cxx-compiler c-compiler cmake llvmdev tbb-devel tbb astroquery boost-cpp sleef xtensor xtensor-blas blas blas-devel 'fmt=8.1.*' 'spdlog=1.10.*' python=3.11 pybind11 numpy mpmath sympy cloudpickle mppp git make
