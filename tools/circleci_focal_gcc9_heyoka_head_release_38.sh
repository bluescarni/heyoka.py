...
conda create -y -q -p $deps_dir python=3.8 git pybind11 numpy mpmath cmake llvmdev tbb-devel tbb astroquery boost-cpp mppp sleef 'fmt=8.0.*' 'spdlog=1.9.2' sphinx myst-nb matplotlib sympy scipy pykep cloudpickle sphinx-book-theme
